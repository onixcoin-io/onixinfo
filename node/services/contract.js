...
const totalSupplyABI = Solidity.orc20ABIs.find(abi => abi.name === 'totalSupply')
const balanceOfABI = Solidity.orc20ABIs.find(abi => abi.name === 'balanceOf')
const ownerOfABI = Solidity.orc721ABIs.find(abi => abi.name === 'ownerOf')
const transferABI = Solidity.orc20ABIs.find(abi => abi.name === 'transfer')
const TransferABI = Solidity.orc20ABIs.find(abi => abi.name === 'Transfer')
...
class ContractService extends Service {
  ...
  async start() {
    ...
    this.#ORC20 = this.node.getModel('orc20')
    this.#ORC20Balance = this.node.getModel('orc20_balance')
    this.#ORC721 = this.node.getModel('orc721')
    this.#ORC721Token = this.node.getModel('orc721_token')
    ...
    for (let transaction of block.transactions) {
      for (let i = 0; i < transaction.outputs.length; ++i) {
        ...
        if (output.scriptPubKey.type === OutputScript.EVM_CONTRACT_CREATE) {
          ...
          if (contract && contract.type === 'orc20') {
  ...
  async _syncContracts() {
    ...
    if (contractsToRemove.length) {
      await this.#db.query(sql`
        DELETE contract, tag, orc20, orc20_balance, orc721, orc721_token
        FROM contract
        LEFT JOIN contract_tag tag ON tag.contract_address = contract.address
        LEFT JOIN orc20 ON orc20.contract_address = contract.address
        LEFT JOIN orc20_balance ON orc20_balance.contract_address = contract.address
        LEFT JOIN orc721 ON orc721.contract_address = contract.address
        LEFT JOIN orc721_token ON orc721_token.contract_address = contract.address
  ...
  async _createContract(address, vm) {
    ...
    if (isORC721(code)) {
      let [nameResult, symbolResult, totalSupplyResult] = await this._batchCallMethods([
        {address, abi: Solidity.orc721ABIs.find(abi => abi.name === 'name')},
        {address, abi: Solidity.orc721ABIs.find(abi => abi.name === 'symbol')},
        {address, abi: Solidity.orc721ABIs.find(abi => abi.name === 'totalSupply')}
      ])
      try {
        ...
        contract.type = 'orc721'
        await contract.save()
        await this.#ContractCode.bulkCreate([{sha256sum, code}], {ignoreDuplicates: true})
        await this.#ContractTag.create({contractAddress: address, tag: 'orc721'})
        await this.#ORC721.create({
    ...
    } else if (isORC20(code)) {
      let [
        nameResult, symbolResult, decimalsResult, totalSupplyResult, versionResult
      ] = await this._batchCallMethods([
        {address, abi: Solidity.orc20ABIs.find(abi => abi.name === 'name')},
        {address, abi: Solidity.orc20ABIs.find(abi => abi.name === 'symbol')},
        {address, abi: Solidity.orc20ABIs.find(abi => abi.name === 'decimals')},
        {address, abi: Solidity.orc20ABIs.find(abi => abi.name === 'totalSupply')},
        {address, abi: Solidity.orc20ABIs.find(abi => abi.name === 'version')}
      ])
      try {
        let version
        try {
        ...
        await contract.save()
        await this.#ContractCode.bulkCreate([{sha256sum, code}], {ignoreDuplicates: true})
        await this.#ContractTag.create({contractAddress: address, tag: 'orc20'})
        await this.#ORC20.create({
          contractAddress: address,
          name,
          symbol,
          decimals,
  ...
  async _processReceipts(block) {
    ...
    for (let addressString of totalSupplyChanges) {
      ...
      if (contract) {
        ...
        if (contract.type === 'orc20') {
          await this.#ORC20.update({totalSupply}, {where: {contractAddress: address}})
        } else {
          await this.#ORC721.update({totalSupply}, {where: {contractAddress: address}})
        }
      }
    }
  }
  ...
  async _updateBalances(balanceChanges) {
    ...
    if (operations.length) {
      await this.#ORC20Balance.bulkCreate(operations, {updateOnDuplicate: ['balance'], validate: false})
    }
  }
  ...
  async _updateTokenHolders(transfers) {
    ...
    await this.#ORC721Token.bulkCreate(operations, {updateOnDuplicate: ['holder'], validate: false})
  }
}

function isORC20(code) {
  return code.includes(balanceOfABI.id)
    && code.includes(transferABI.id)
    && code.includes(TransferABI.id)
}

function isORC721(code) {
  return code.includes(balanceOfABI.id)
